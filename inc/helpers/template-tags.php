...
function aquila_posted_on() {
	...
	$posted_on = sprintf(
		...
		'<p class="mb-0 pb-0">' . $time_string . '</p>'
	);
...
function aquila_posted_by() {
	$byline = sprintf(
		esc_html_x( ' Yazar: %s', 'post author', 'aquila' ),
...
function aquila_excerpt_more( $more = '' ) {
	...
	if ( ! is_single() ) {
		$more = sprintf( '<a class="aquila-read-more text-white" href="%1$s"><button class="mt-2 btn-sm btn btn-info">%2$s</button></a>',
